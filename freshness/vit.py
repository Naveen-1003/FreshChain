...
class PatchEmbedding(layers.Layer):
    ...
    def __init__(self, patch_size, embed_dim, **kwargs):
        ...
        self.cls_token = None  # CLS token (optional, but standard ViT)

    def build(self, input_shape):
        _, height, width, _ = input_shape
        self.num_patches = (height // self.patch_size) * (width // self.patch_size)

        # 1. Add CLS Token (trainable parameter)
        ...
        self.position_embedding = layers.Embedding(
            input_dim=self.num_patches + 1,  # +1 for the CLS token
    ...
    def call(self, images):
        ...
        # Prepend the CLS token (copied across the batch dimension)
        cls_token_batch = tf.broadcast_to(self.cls_token, [tf.shape(x)[0], 1, self.embed_dim])
        x = tf.concat([cls_token_batch, x], axis=1)  # (B, num_patches + 1, embed_dim)
...
# --- 2. Model Components: Standard Transformer Encoder Block ---

class TransformerEncoderBlock(layers.Layer):
    """A standard ViT Encoder block (Attention + MLP)."""
    ...
    def call(self, x):
        # Layer 1: Multi-Head Attention (Pre-norm residual)
        norm_output_1 = self.norm1(x)
        attn_output = self.attn(norm_output_1, norm_output_1)
        x = x + attn_output  # Residual connection

        # Layer 2: MLP (Pre-norm residual)
        norm_output_2 = self.norm2(x)
        mlp_output = self.mlp(norm_output_2)
        x = x + mlp_output  # Residual connection
        return x


# --- 3. The Vision Transformer Model (ViT) ---
...
class ViT(keras.Model):
    def __init__(self, img_size, patch_size, num_classes, embed_dim, depth, num_heads, mlp_dim, **kwargs):
        ...
        # Create 'depth' INDEPENDENT encoder blocks
        self.encoder_blocks = [
    ...
    def call(self, inputs):
        # 1. Patching + Positional Embedding + CLS Token
        x = self.patch_embed(inputs)  # (B, num_patches + 1, embed_dim)

        # 2. Transformer Encoder Blocks
        for encoder_block in self.encoder_blocks:
            x = encoder_block(x)

        # 3. Final Classification (using the CLS token)
        S_T = self.final_norm(x)
        pooled_output = S_T[:, 0, :]  # Extract only the CLS token
...
# NOTE: The data loading functions below are identical to the previously confirmed stable version.
...
def run_experiment(data_dir="dataset"):
    HPARAMS = {
        'img_size': 128,  # ViT-Small/16 Input Resolution
        'patch_size': 16,
        'embed_dim': 384,  # ViT-Small/16 Hidden Size
        'depth': 6,  # ViT-Small/16 Depth
        'num_heads': 6,  # ViT-Small/16 Heads
        'mlp_dim': 1536,  # ViT-Small/16 MLP Size (4*384)
    ...
    vit_model = ViT(  # Using the new ViT class
    ...
    vit_model.build(input_shape=(HPARAMS['batch_size'], HPARAMS['img_size'], HPARAMS['img_size'], 3))

    print("\n✅ ViT-Small/16 Model Initialized")
    print(f"Total trainable parameters: {vit_model.count_params()}")
    ...
    model_save_path = "best_vit_small_model.keras"
    checkpoint_callback = ModelCheckpoint(
        filepath=model_save_path,
        monitor='val_accuracy',
        mode='max',
        save_best_only=True,
        verbose=1  # Prints save message to console.
    )
    callbacks = [checkpoint_callback]

    # 6. Train Model
    print("\n--- Starting ViT-Small/16 Training ---")
    vit_model.fit(
        train_ds,
    ...
    custom_objects = {"PatchEmbedding": PatchEmbedding, "TransformerEncoderBlock": TransformerEncoderBlock, "ViT": ViT}
    best_model = None
